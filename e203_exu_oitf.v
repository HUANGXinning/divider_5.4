...
module e203_exu_oitf (
  ...
  input  dis_ena, //派遣一个长指令的使能信号，该信号将用于分配一个OITF表项
  ...
  input  disp_i_rs1fpu, //当前派遣指令第一个源操作数是否要读取浮点通用寄存器组
  input  disp_i_rs2fpu, //当前派遣指令第二个源操作数是否要读取浮点通用寄存器组
  input  disp_i_rs3fpu, //当前派遣指令第三个源操作数是否要读取浮点通用寄存器组
  input  disp_i_rdfpu, //当前派遣指令结果寄存器是否要写回浮点通用寄存器组
  input  [`E203_RFIDX_WIDTH-1:0] disp_i_rs1idx, //当前派遣指令第一个源操作数的索引
  input  [`E203_RFIDX_WIDTH-1:0] disp_i_rs2idx, //当前派遣指令第二个源操作数的索引
  input  [`E203_RFIDX_WIDTH-1:0] disp_i_rs3idx, //当前派遣指令第三个源操作数的索引
  input  [`E203_RFIDX_WIDTH-1:0] disp_i_rdidx, //当前派遣指令的结果寄存器的索引
  input  [`E203_PC_SIZE    -1:0] disp_i_pc, //当前派遣指令的PC

  output oitfrd_match_disprs1, //派遣指令源操作数一和OITF任一表项的结果寄存器相同
  output oitfrd_match_disprs2, //派遣指令源操作数二和OITF任一表项的结果寄存器相同
  output oitfrd_match_disprs3, //派遣指令源操作数三和OITF任一表项的结果寄存器相同
  output oitfrd_match_disprd, //派遣指令结果寄存器和OITF任一表项的结果寄存器相同
...
//由于OITF本质上是一个FIFO，因此需要生成FIFO的写指针

  wire alc_ptr_ena = dis_ena; //派遣一个长指令的使能信号，作为写指针的使能信号
  wire ret_ptr_ena = ret_ena; //写回一个长指针的使能信号，作为读指针的使能信号
